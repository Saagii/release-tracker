...
export class StatusService {
    ...
    */
    getStatusStyle(statusValue: string): any {
        ...
        if(['Failed', 'Rejected', 'Stopped', 'Terminated'].includes(statusValue)) {
            return 'bg-red-600 text-red-200 font-bold border border-red-400'
        }
        if(['Delayed'].includes(statusValue)) {
        ...
        if(['Initiated'].includes(statusValue)) {
            return 'bg-yellow-100 text-yellow-600 font-bold border border-yellow-200'
        }
    }
